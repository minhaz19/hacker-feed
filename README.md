...
The legacy Bridge serialized all JS-to-native communication as JSON and posted it asynchronously across a message queue, meaning JS could never make a synchronous native call. This made animations and gesture-driven interactions inherently laggy because every frame update had to survive that async round-trip. JSI replaces the bridge with a C++ layer that gives JS a direct reference to native host objects, enabling synchronous calls with zero serialization overhead. TurboModules build on JSI to lazily load native modules and communicate through it instead of the old bridge registry. Fabric is the new rendering engine: JS directly mutates a shared C++ shadow tree rather than diffing and posting a serialized patch, which lets the UI thread respond without an async handoff. Together these unlock React 18 concurrent features — useTransition, startTransition — which were impossible on the bridge because the UI thread had no synchronous visibility into the JS scheduler.

---

### Q2: Diagnosing a Janky FlatList Rendering 500 Items on Mid-Range Android

First, open Flipper's Performance Monitor or the in-app Perf overlay and distinguish whether JS frames or UI frames are dropping — that determines which thread the bottleneck lives on. If JS is the culprit, profile renderItem cost; if UI, look at view count and GPU overdraw via Android's GPU Profiler. Second, wrap renderItem in useCallback and memoize the item component with React.memo so unrelated parent state changes don't cascade re-renders through the list. Third, tune FlatList's windowing props — lower initialNumToRender to what fills the screen, set windowSize={5}, and enable removeClippedSubviews on Android to detach off-screen views from the native hierarchy. Fourth, flatten the item's view hierarchy and move all styles to StyleSheet.create; deep nesting causes overdraw that tanks GPU performance on mid-range hardware. If those steps are insufficient, replace FlatList with Shopify's FlashList, which recycles a fixed pool of native cell views instead of creating a new JS+native view pair per item.
